...
const SuccessLogo = ({ className }) => {
  ...
  useEffect(() => {
    ...
    // Set up the dotted circle with the specified CSS properties
    if (circle) {
      circle.style.strokeDashoffset = "100";
      circle.style.transition = "stroke-dashoffset 3000ms";
    }

    // After a short delay, animate the stroke-dashoffset to 0 to make dots spin
    setTimeout(() => {
      if (circle) {
        circle.style.strokeDashoffset = "0";
      }
    }, 500); // 0.5 second delay before starting animation

    // After 1 seconds, draw in the solid circle
    setTimeout(() => {
      if (circle) {
        circle.style.strokeDasharray = "100, 0";
        circle.style.strokeDashoffset = "0";
        circle.style.transition = " stroke-dashoffset 3000ms";
        circle.style.transition = "stroke 0.5s";
        circle.style.stroke = "#245c4f";

        const paths = svg.querySelectorAll(".logo-path");
        paths.forEach((path) => {
          path.style.transition = "opacity 0.35s";
          path.style.opacity = "0";
        });
      }
    }, 1000); // 1 second delay before drawing solid circle

    // After 1.1 seconds, draw in the checkmark (staggered after letters disappear)
    setTimeout(() => {
      // Draw in the checkmark
      const checkmark = svg.querySelector(".checkmark");
      if (checkmark) {
        checkmark.style.opacity = "1";
        const checkLength = checkmark.getTotalLength();
        checkmark.style.strokeDasharray = `${checkLength}, 1000`; // Single long dash with huge gap
        checkmark.style.strokeDashoffset = `${checkLength}`; // Start with checkmark not visible
        checkmark.style.transition = "stroke-dashoffset 1s ease-in-out";
        checkmark.style.strokeDashoffset = "0"; // Animate to 0 to draw in the checkmark
      }
    }, 1100); // 1.1 second delay before drawing checkmark
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* NK Letters - filled version */}
      <g fill="#245c4f" stroke="none">
      ...
      {/* Dotted circle around the letters */}
      <circle
      ...
      {/* Green checkmark that appears after NK letters disappear */}
      <path
        className="checkmark"
        d="M18 32 L26 40 L42 20"
        fill="none"
        stroke="#245c4f"
        strokeWidth="4"
        strokeLinecap="round"
